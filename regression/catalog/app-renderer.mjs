...
const rows = [
  ...
  ["CONTEXT-NATIVE-NO-OVERRIDE-002", "Context 是当前 thread 的非缩容实时控制", "Native Context 不覆盖官方字段", ["L1", "L3", "L4"], ["all"]],
  ["CONTEXT-IDLE-MONOTONIC-003", "Context 是当前 thread 的非缩容实时控制", "空闲 thread 同档或增大立即生效", ["L1", "L3", "L4"], ["all"]],
  ["CONTEXT-ACTIVE-SAFE-BOUNDARY-004", "Context 是当前 thread 的非缩容实时控制", "正在生成时在当前 thread 的安全边界生效", ["L1", "L3", "L4"], ["all"]],
  ["CONTEXT-QUEUE-COALESCE-005", "Context 是当前 thread 的非缩容实时控制", "连续选择只应用最后一个合法目标", ["L1", "L3"], ["all"]],
  ["CONTEXT-SHRINK-REJECT-006", "Context 是当前 thread 的非缩容实时控制", "缩小 Context 在修改 thread 前被拒绝", ["L1", "L3", "L4"], ["all"]],
  ["CONTEXT-NATIVE-COMPARABLE-007", "Context 是当前 thread 的非缩容实时控制", "Native 参与相同的容量判定", ["L1", "L3", "L4"], ["all"]],
  ["CONTEXT-NATIVE-UNKNOWN-008", "Context 是当前 thread 的非缩容实时控制", "Native 容量更小或无法可靠解析", ["L1", "L3"], ["all"]],
  ["CONTEXT-IDEMPOTENT-009", "Context 是当前 thread 的非缩容实时控制", "选择当前 Context 是幂等操作", ["L1", "L3"], ["all"]],
  ["CONTEXT-WINDOW-MAPPING-010", "Context 是当前 thread 的非缩容实时控制", "272K 和 450K 映射准确", ["L1", "L3", "L4"], ["all"]],
  ["CONTEXT-PENDING-FRESH-USAGE-011", "Context 是当前 thread 的非缩容实时控制", "热切换期间展示区分配置值与有效值", ["L1", "L3", "L4"], ["all"]],
];
